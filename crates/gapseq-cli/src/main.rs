//! `gapseq` command-line entry point.
//!
//! Mirrors the bash-dispatched `gapseq` subcommand structure. Every subcommand
...
///
/// New subcommands arrive per milestone — see `gapseq-rs/README.md`. Anything
/// not yet wired up returns a "not implemented yet" error instead of silently
/// doing nothing.
#[derive(Debug, Parser)]
#[command(name = "gapseq", version, about, long_about = None)]
pub struct Cli {
    /// Override the gapseq reference-data directory (default: auto-detect).
...
pub enum Cmd {
    ...
    /// Print resolved paths and tool versions; used by `gapseq test`.
