//! `gapseq adapt` — add or remove reactions / pathways on an existing model.
//!
//! # Example
//!
//! ```bash
...
//!   `getDBhit`; our dbhit index is in `gapseq-find` but plumbing it into
//!   adapt is deferred).

use clap::Parser;
use gapseq_core::{CompartmentId, Metabolite, Model, Reaction, StoichMatrix};
use gapseq_db::{load_seed_reactions, pathway::PwySource, PathwayTable, SeedRxnRow};
use gapseq_fill::{
...
struct IdResolver<'a> {
    pathways: &'a [gapseq_db::PathwayRow],
}
...
fn add_reactions(model: &mut Model, seeds: &[String], seed_rxns: &[SeedRxnRow]) -> Vec<String> {
    ...
    for seed in seeds {
        ...
        if let Some(row) = seed_rxns.iter().find(|r| r.id.as_str() == seed) {
            gapseq_draft::builder::add_seed_reaction(model, row, Some(10));
            added.push(format!("{seed}_c0"));
        }
    }
    gapseq_draft::builder::rebuild_s_matrix(model);
    added
}
...
fn remove_reactions(model: &mut Model, seeds: &[String]) -> Vec<String> {
    ...
    let present: Vec<String> = model
        ...
        .collect();
    gapseq_fill::drop_reactions(model, &present.iter().cloned().collect());
    present
}
...
fn adapt_growth(
    ...
    let medium: Vec<gapseq_fill::MediumEntry> = model
        .rxns
        .iter()
        .filter(|r| (r.is_exchange || r.id.as_str().starts_with("EX_")) && r.lb < 0.0)
        .map(|r| gapseq_fill::MediumEntry {
            compound: r
                .id
                .as_str()
