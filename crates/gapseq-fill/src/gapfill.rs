...
pub fn gapfill4(
    ...
    for (seed, row) in &utilized {
        gapseq_draft::builder::add_seed_reaction(&mut filled, row, opts.gs_origin);
        added_ids.push(format!("{seed}_c0"));
        if weights.is_core(seed) {
            added_core.push(seed.clone());
        }
    }
    gapseq_draft::builder::rebuild_s_matrix(&mut filled);
...
pub fn drop_reactions(model: &mut Model, rxn_ids: &HashSet<String>) {
    ...
    model.rxns = new_rxns;
    model.s = gapseq_core::StoichMatrix::from_triplets(n_mets, n_rxns_new, triplets);
}
